...
/* Im ALLBUS 2016 sind die fehlenden Werte nach einem einheitlichen Schema codiert, das
   auf der Internetseite des ALLBUS (http://www.gesis.org/allbus) heruntergeladen werden kann.
   ...
   In Stata ist es nicht möglich, einen Wertebereich einer Variable als fehlend zu definieren. 
   Stattdessen müssen Werte, die von der Analyse ausgeschlossen werden sollen, auf systemdefiniert fehlende 
   Werte kodiert werden.
   Um auch in den Stata Datensätzen des ALLBUS mit fehlenden Werten arbeiten zu können, 
   ist es nötig die Codes für fehlende Werte in die Stata eigene Missing Value Logik zu übersetzen. 
   Dies wird mit diesem Do-File ermöglicht.
